...
(defun trial(pat exp &optional (env (make-stack :size 20) wasitprovided))
 ...
 (let ((res (if  (match pat exp) ;; match is the matching program. env is global, result bindings.
...
(defun match (pat exp &optional (condition #'truth))
  ...
  (m1 pat exp nil nil condition)))

(defun isanyopt(x)
  ;; fast scan of pattern to see if there are any Optionals ANYwhere
...
(defun m1 (p e name gh c)
  ...
  (let* ((hp (Head p))
    ...
    (if (and *anyopts* (hasoptionalp p))(return-from m1 (m1 (remopts p) e name gh c)))
...
(defun hasoptionalp(p)
  (if (consp p)
  (some #'(lambda(r)(and (consp r)(eq (car r) 'Optional))) p)))

(defun remopts (p)			; restructure pattern to remove Optional in favor of Alternatives.
  ...
  (let ((r (append p nil))
	...
	((null rp) p)			; won't exit this way..
      (cond ((and(consp (car rp))
	     ...
	     (setf default (caddar rp)) ;; the default
	     ;(format t "~%default=~s" default)
	     (setf (car rp) pattern) ;; the pattern
	     ;(format t "~%sp=~s" sp)

	  ;;      (setf (car sp) (caddar sp)) ;; the default
	     (setf (car sp) '(Sequence))
...
#| not good enough?
 ...
 maybe introduce an 'action' recognized by m1 m2 etc
 ...
 or perhaps an action that is not lisp, but some simple matcher language.
 so the Optional would become ..
(Alternatives (f a b (Power x (Pattern n (Blank))) c)
                        (Action (f a b x c) (Bind n 1) ));  need to implement Action processing.

;; this could work for Power, but also Times and Plus. Example:

(f a b (Times  (Optional (Pattern n (Blank)) 1) x )  c) could be

(Alternatives (f a b (Times (Pattern n (Blank)) x) c)
...
OOO
Action (and Bind)?must be recognized in m1 and m2. maybe something like.. 
((eq head 'Action)
 (let* ((apat (cadr p))
	(act (caddr p))			; action clause
	(theact (car act))
	(bindvar (cadr act))
	(bindval (caddr act)))
   ;; assume it is a bind
   (if (eq 'Bind theact)
       (m1/2 bindvar bindval name gh #'(lambda()(m1/2 apat e name gh c)))
     (format t "~%invalid action specified in ~s" act))))

Without going deeper we can do this

(f a b (Alternatives (Power x (Pattern n (Blank))) (Action x (Bind n 1))) c)
...
(defun m2 (p e name gh c)  
  ...
  (let* ((hp (Head p))
     ...
     (if (and *anyopts* (hasoptionalp p))(return-from m2 (m2 (remopts p) e name gh c)))
    (dformat t "~%ha=~s" ha) ;; attributes of head of p
    (cond 
     ((null p)(if (null e) (funcall c) nil)) ; check the conditions.
     ((atom p)(and (equal p e)(bindifposs name e) (funcall c)))
 ...
 (dformat t "~%mlist env=~s" (env2alist env))
  (cond ((null pl) 
	...
	((eq (caar pl) 'Optional)
	 ...
	 #+ignore 	 (mlist-option pl el name gh condition)
	 )
...
(defun mlist-option(pl el name gh condition)
  ...
  (if ;nil				; 
      (mlist (cons (second (car pl))(cdr pl)) el name gh condition)
	 ...
	 (cond (found 
		...
		(and(equal (meval thedefault)(meval val)) ; meval to be safe?
	       ...
	       (t (cond ((mlist (cons thepat (cdr pl)) el thename gh condition) 
			 (schange env thename thedefault) t)
			(t
    ...
    (labels ((runtest1(h)
	       (cond  ((null h)(return-from showtest 'Done))
		      ...
		      (t   (dispb (list 'List (list 'Matchpat (cadar(cdar h))
			    (cadr(cadr(cdar h))))  (format nil " --> ~s"  (eval (car h)))))))
	       (runtest1 (cddr h) )))
      (runtest1 h))))
...
(defparameter
 ...
 (trial '(gh (Pattern a (Blank)) (Pattern b (Blank Sin))) '(gh (Sin y))) ; gh is flat and orderless
