...
;;; bfappend.lisp makes it all accessible from MockMMA.

;;; much of this file and bfelem  (and more) can be done more efficiently
;;; by the mpfr library, should that be available in the host lisp.
;;; see http;//www.cs.berkeley.edu/~fateman/generic for examples.
  ...
  ;; log[2](10) is 3.321928 ...
   ...
   (let ((bigfloat-bin-prec 
	  (max bigfloat-bin-prec
	       (+ (ceiling 3.321193 (max 1(intlen exp)))
		  (intlen frac)))))
     ;; compute frac*10^exp
  ...
  (typecase x
	     (bigfloat
	      (cond((= bigfloat-bin-prec (bigfloat-precision x)) x)
		   ...
		   (bigfloat-fraction x)
		   (bigfloat-exponent x)))))
	     (integer (intofp x)) ;bignum or fixnum
	     	 ;; this is the common-lisp rational number type
	     (rational
	      (bigfloat-/ ;; can't lose on under/overflow
	       (intofp (numerator x))
	       (intofp (denominator x))))
	     (float ;single, double or other float
	      (bigfloat-convert (rational x)))
	     (otherwise x))) ;; if it can't be converted, return x? probably not useful
