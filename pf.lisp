...
(defun prec (x)
  (cond ((consp x) 260)  ;; I guess.. this is for operators
	((not(symbolp x)) 260)
	(t (or (get x 'stringprec) 260))))
...
(defun BuildFormat (x)
  ...
  (let* ((h (|Head| x))  ;; could be fixnum, or Plus, or ..
	 (fun (if (symbolp h)(get h 'formatter) #'deflt-buildformat)))
;; (format t "~% formatter is ~s" fun)
    (cond 
    ...
    ;; then use the default atom-head formatter.
     ((atom h)(if fun  (funcall fun x)(deflt-buildformat x)))
...
(defun symbolformatter(x) (if x x '|False|)) ;; nil is False
...
(defun optionaldefaultformatter (x)
  (let ((width 0)
	(maxtoph 1)
        (maxbotmh 0)
	(hoffset 0)
	...
	((null args)
	 (MakeHForm (- width 3)
		    (+ maxtoph maxbotmh) hoffset voffset '|Optional|
		    (prec '|Optional|) 'none
		    (cdr (nreverse ls))))
	(setq ret_format (BuildFormat (car args)))
	(cond ((atom ret_format)
	       ...
	       (setq ls `( ,ret_format ":",@ls)))
	      (t
	       (if (lessprec ret_format '|Optional|)
		   (setq ret_format (parenformatter ret_format)))
	       (setq heightpacket (TopBotHs ret_format maxtoph maxbotmh))
	       (setq maxtoph (car heightpacket))
	       (setq maxbotmh (cdr heightpacket))
...
(setf (get '|Optional| 'formatter) 'optionalformatter)
(setf (get '|Optional| 'stringprec) 152) ;guess  2/96RJF
(setf (get '|Optional| 'stringform) ".")

(defun optionalformatter(x) 
  ;; either (Optional (Pattern n ...) 1)  ==   n_:1
  ;; or 
  ;; (Optional (Pattern n ...))  ==   n_.
  (if (cddr x) 
      (optionaldefaultformatter x)
    ;(genformatter x '|Optional|)
    ;;(genformatter (cadr x) ":" (caddr x))
    (postformatter x )))

;;  (setf (get '|Optional| 'formatter) 'postformatter)
...
;;          and convert it to a real number string. e.g. (Real 1 2).
;;          Also works for floats.

(defvar realnumformatstring "~f")

(defun realformatter (x)
  (cond ((numberp x)(format nil realnumformatstring x))
	(t (format nil "~D.~D" (cadr x)(caddr x)))))
...
(defun complexnumformatter (x)
  (let ((rp 0)(ip 0))
    (if (atom x) ;; actually a lisp complex number
	(setf rp (realpart x) ip (imagpart x))
      (setf rp (cadr x) ip (caddr x)))
;; (format t "~% rp=~s ip=~s" rp ip)
      (let ((imag (cond ((eql ip 0) nil)
			((eql ip 1) 'I)
		    (t `(|Times| ,ip I)))))
	(BuildFormat (cond ((null imag)rp)
			   ((eql 0 rp) imag)
		       (t (list '|Plus| rp imag)))))))
...
;; Jan 2011
(setf (get '|Pattern| 'formatter) 'patternformatter)
(defun patternformatter(x)(format nil "~a~a" (cadr x)(bformatter (caddr x))))
(setf (get '|Blank| 'formatter) 'bformatter)
(setf (get '|BlankSequence| 'formatter) 'bformatter)
(setf (get '|BlankNullSequence| 'formatter) 'bformatter)

;; I don't really knonw how to do this so it builds stuff recursively 2/2011 RJF
...
(defun bformatter(x)
  ...
  (format nil"~a~a"
	  ...
	  (if (member (car x) '(|Blank| |BlankSequence| |BlankNullSequence|) :test 'eq)
	      (if (cdr x) (cadr x) "")
	    
	    x))))
;;(setf (get 'PatternTest 'formatter) 'pattestformatter)
;;(defun pattestformatter(x)  ;; PatternTest[a,b]   is  a?b
;;  )


;; Optional[Pattern[n,Blank[]],1]   is  n_:1.   Must be Blank[]

	      

(setf (get '|Symbol| 'formatter) #'symbolformatter)
(setf (get 'rat 'formatter) #'(lambda(x) (BuildFormat(outof-rat x))))
(setf (get '|Rat| 'formatter) #'(lambda(x) (BuildFormat(outof-rat x))))
(setf (get '|Complex| 'formatter) #'complexnumformatter)
